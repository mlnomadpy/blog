...
def e3_big():
    ...
    Ks = [64, 256] if SMOKE else [int(k) for k in os.environ.get("E3B_KS", "256,1024").split(",")]
    ns = [4000] if SMOKE else [
        len(Xtr) if v == "full" else int(v)
        for v in os.environ.get("E3B_NS", "64000,256000,full").split(",")
    ]
    ep_full = int(os.environ.get("E3B_EPOCHS", "8"))
    rows = []
    for K in Ks:
        lr = sweep_lr(Xtr[:64000], ytr[:64000], Xva, yva, K, (3e-3, 1e-3, 3e-4), 0,
                      2 if SMOKE else 6, 512, "bce", eps=eps3)
        for n in ns:
            epochs = 2 if SMOKE else (16 if n <= 64000 else ep_full)
            accs = []
