...
kern_curve = {'loss': [], 'linear': [], 'centroid': [], 'cka': [], 'ckaTarget': [], 'spectra': []}
for seed in [0, 1, 2]:
    st = student_net(seed)
    if seed == 0:
        def per_epoch(m, ep):
...
pick = np.concatenate([rng.choice(np.where(yte == c)[0], 24, replace=False) for c in range(10)])
rng.shuffle(pick)
...
p_h = unit(np.asarray(jax.nn.softmax(jnp.asarray(zt_te[pick[h_idx]]) / TEMP, axis=1)))
G_h = p_h @ p_h.T
