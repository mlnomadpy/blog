...
GEOMETRY = os.environ.get("GEOMETRY", "0") == "1"
SEEDS = (0,) if (SMOKE or NORMSWAP or GEOMETRY
                 or os.environ.get("TELEMETRY", "0") == "1") else (0, 1, 2)
...
def run_variant(variant, seed, lr=None):
    ...
    if GEOMETRY:
        # the geometry export: trained q/k per layer x head on the fixed
        # window, so the partition of query space (who wins where, nearest-
        # vs-aligned, scaling behaviour) can be measured offline.
