...
These steps must happen outside the repo:
...
Deploys run on GitHub Actions — `.github/workflows/deploy.yml` builds on every
push to `master` and pushes `dist/` to the `gh-pages` branch. GitHub Pages
serves that branch at `https://tahabouhsine.com/blog/`.

To deploy from your machine instead (e.g. testing a deploy from a feature
branch), the legacy script still works:
