...
*The second feature layer is four lines of `np.minimum`. That is not a summary of it, that is the layer: junctions, continuations, bends and stripes are each one pointwise AND over the hand-built first layer's maps, and the classifier sitting on top is the same [Flax NNX](https://flax.readthedocs.io/en/latest/api_reference/flax.nnx/module.html) module of placed k-means prototypes as before, with no training loop anywhere in the post. The [explainer](/blog/depth-by-construction/) reports what a century of vision science buys when you spend it this way: 82.9% for both layers, against 83.3% for one. The code is short enough to see why.*
...
A junction is the whole idea in one picture: take the map of one orientation and the map of another, and keep only where both fire. That pointwise minimum is a new detector, computed from layer 1 the same way layer 1 was computed from pixels, with nothing fit.

<figure class="jax-fig jax-fig-wide">
  <img src="/blog/depth-junction-build.gif" alt="Three panels on a real bag: the 30-degree edge map, the 90-degree edge map, and their pointwise minimum, a 30+90 junction detector that fires only at the corners where both orientations are present" loading="lazy" width="946" height="429" />
  <figcaption>The entire mechanism of layer 2, on a real garment. Left, the 30° edge map of a bag; middle, its 90° edge map; right, their pointwise minimum, a detector that is bright only where <em>both</em> orientations are present in the same cell, which is what a junction is. This is <code>np.minimum(e1, e2)</code>, the cheapest faithful AND of two non-negative energies, no parameter fit. The animation reveals the garment, the two edge maps, and their minimum in computational order.</figcaption>
</figure>
...
It prints **82.9%**, a drop of 0.4 points from a layer of century-old vision science. Iterating as we would on any model, with each redesign expressing a theory of what broke, does not help: junctions alone score **83.5%** (the best of six designs), and the rest land at 82.8, 83.0, 82.9, and, for a geometric-mean AND and a two-cell reach, 83.2 and 81.8. Six theories, six refutations by a few tenths.
...
<figure class="jax-fig jax-fig-wide">
  ...
  <figcaption>The researcher's sweep, every number from the run. Six design iterations on the layer-2 vocabulary: junctions only (83.5, best, in teal), + continuations (82.8), + bends (83.0), + stripes (82.9), geometric-mean AND (83.2), two-cell reach (81.8). No theory rises above the layer-1 line by more than two tenths; the head shrugs at every new word offered.</figcaption>
</figure>

<figure class="jax-fig jax-fig-wide">
  <img src="/blog/depth-ladder-build.gif" alt="A horizontal accuracy ladder on the 78 to 87 percent range showing blue built-by-hand rungs clustered near 83 and the fully trained network alone in orange at 85.7" loading="lazy" width="936" height="576" />
  <figcaption>The construction ladder, built (blue) against trained (orange), on the axis that magnifies the photo finish: raw pixels 79.0, layer 1 by hand 83.3, layer 2 alone 78.8, both layers 82.9, best of six 83.5, and the trained head on built features 83.2, all sitting on top of one another, while only the fully trained network keeps its 85.7. The staged reveal separates the labels, the hand-built cluster, and the final trained margin.</figcaption>
</figure>
...
<figure class="jax-fig jax-fig-wide">
  <img src="/blog/depth-layer2-build.gif" alt="Six dimmed layer-1 edge maps of a trouser on the left marked discarded, three bright layer-2 relation maps in the middle marked kept, and a gauge reading 78.8 percent on the right" loading="lazy" width="946" height="473" />
  <figcaption>Classifying from relations only, the real maps and the real number. Left, the six layer-1 edge maps, dimmed because they are discarded; middle, the hand-built layer-2 detectors (junctions, a continuation) that are kept; right, the gauge at the measured 78.8% the head reaches on layer 2 alone, close to the raw-pixel 79. The words are real words with real signal, even with the edges thrown away. The animation moves from discarded edge maps to retained relations and then the measured gauge.</figcaption>
</figure>
