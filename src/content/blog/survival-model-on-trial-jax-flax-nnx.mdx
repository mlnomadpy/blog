...
description: "Build the Yat DeepSurv trunk and Cox loss in Flax NNX, recover exact prototype contributions and explicit row edits, then run the LR-fair five-dataset benchmark with concordance, calibration, Brier, AUC, and classical baselines."
...
*Two trunks, one pipeline. The [explainer](/blog/survival-model-on-trial/) put a deep Yat-kernel survival model on trial across five real datasets: viable, in the pack on the mid-sized problems, behind on the small and the near-separable ones, and carrying a theory the baselines do not have. What makes those verdicts worth anything is the plumbing, so the plumbing is what this page is. The Cox partial-likelihood loss. A training loop in which neither net is ever run at a borrowed learning rate. The concordance / integrated-Brier / time-dependent-AUC evaluation, and the classical baselines wired through [scikit-survival](https://scikit-survival.readthedocs.io/). The [Flax NNX](https://flax.readthedocs.io/en/latest/api_reference/flax.nnx/module.html) trunk is the only piece that changes between the two neural models. A research illustration, not a clinical tool.*

## The trunk is the only thing that changes

A DeepSurv is a loss with a hole in it: any network that maps a patient to one scalar log-risk can be dropped into the [Cox partial likelihood](/blog/survival-model-on-trial/) and trained. The standard recipe fills the hole with a ReLU MLP; the white-box model fills it with a bank of Yat kernels, and nothing else about the pipeline moves. So both trunks are two short modules with the same signature.
...
The `Winit` argument is where the legibility is decided. The prototypes are seeded on real patients (k-means centroids of the training cohort), so from the first step every hidden unit is a point in patient space rather than an opaque direction. The standard DeepSurv is the same file with a two-layer ReLU MLP in place of `YatLayer`, and it has no `Winit` argument to pass, because there is nowhere in it for a patient to sit.
...
<figure class="jax-fig jax-fig-wide">
  ...
  <figcaption>Risk stratification forming during a real training run. The three curves are Kaplan-Meier estimates on the low, mid and high risk tertiles the model's current log-risks define; they begin overlapping at epoch 0 and pull apart as the Cox loss sorts the cohort.</figcaption>
</figure>
...
<figure class="jax-fig">
  ...
  <figcaption>The Yat prototypes settling onto the cohort during the same real run. Grey dots are patients in a fixed 2D shadow of the input space; each ring is a real prototype vector at that epoch, orange if its readout raises risk, blue if it protects.</figcaption>
</figure>
...
<figure class="jax-fig jax-fig-wide">
  ...
  <figcaption>Yat concordance index against prototype count K, k-means (orange) vs random (blue) seeding, mean and seed spread. Accuracy is flat in K on both datasets.</figcaption>
</figure>
...
<figure class="jax-fig jax-fig-wide">
  <img src="/blog/trial-calibration-read.gif" alt="Predicted survival curves against dashed Kaplan-Meier observed curves for three risk tertiles, for the Yat and standard DeepSurv on METABRIC" loading="lazy" width="720" height="290" />
  <figcaption>Predicted (solid) vs Kaplan-Meier observed (dashed) survival by risk tertile on METABRIC. The reliability gap is the mean absolute distance between the two. The models are revealed separately before the reliability gaps are compared.</figcaption>
</figure>

The out-of-distribution meter is a single `max` over the kernel activations, and it does not fire everywhere: scored across five datasets and three constructions, some bars land above 0.5 (the meter flags a stranger) and some land below (it does not, because the shift is mild or a far-tail point still falls near a prototype).
...
<figure class="jax-fig jax-fig-wide">
  ...
  <figcaption>Kernel-max OOD AUROC by dataset and construction, with bootstrap CIs. Above 0.5 the meter flags the construction as strange; the below-0.5 bars are where it does not.</figcaption>
</figure>
...
<figure class="jax-fig jax-fig-wide">
  ...
  <figcaption>Per-covariate in-range check for all 24 prototypes on METABRIC. Green is inside the training min/max, orange outside; the excursions cluster on the binary treatment indicators. Eight prototypes are fully in range.</figcaption>
</figure>
...
## Exact terms and explicit edits

The benchmark metrics assess the fitted predictor. The architecture also exposes its score arithmetic without a second explainer model. For a one-output linear readout,

```python
def decompose(model, x):
    phi = model.yat(x)                              # [batch, K]
    a = model.readout.kernel.value[:, 0]            # [K]
    terms = phi * a[None, :]
    risk = terms.sum(axis=1) + model.readout.bias.value[0]
    return risk, terms

risk, terms = decompose(model, X_test)
np.testing.assert_allclose(risk, np.asarray(model(X_test)).squeeze(), rtol=1e-5)
```

Those are exact additive log-risk terms for the current layer. They are not causal covariate effects. The centers may also leave the empirical patient manifold, which is why the companion de-normalizes and checks every prototype against observed ranges.

An edit is equally explicit. Masking selected readout rows removes their current contribution while leaving all other parameters fixed:

```python
def score_with_mask(model, x, keep):
    phi = model.yat(x)
    a = model.readout.kernel.value[:, 0] * keep
    return phi @ a + model.readout.bias.value[0]
```

This is a reproducible model edit, not retraining and not certified unlearning. Its value is measured by the selectivity of the score and ranking change. Calibration and the kernel-max resemblance meter remain separate diagnostics; neither follows from the additive identity.
