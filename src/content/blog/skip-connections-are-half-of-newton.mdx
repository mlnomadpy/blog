...
Write the honest second-order law with the state split in two, position and velocity:
...
What happened? On two moons, nothing: both update rules sit between 99.85% and 100% accuracy at every depth. The velocity ledger is not a general-purpose upgrade, and it would be dishonest to sell it as one. Euler is a fine integrator when the flow is easy and the step is small.
...
<StabilityCliff caption="Test accuracy against depth for the plain residual net (orange) and the momentum net (blue), three seeds each (band = min to max), total integration time fixed so deeper means smaller steps; the y axis zooms to where the data lives. There is no cliff: the two rules tie to within a point on every task. The fine print sits at the ceiling. Once the plain net is deep enough to be an honest flow (32 and 128 blocks), its band on the rings task never touches 100% again, while the momentum net pins it there. All numbers from scripts/momentum_resnet.py; switch tasks with the selector." />
