...
<figure class="jax-fig">
  <img src="/blog/train-features-untangle.gif" alt="A 2D layout of Fashion-MNIST test features as the backbone trains, going from a tangled blob to separated class clusters, with the constructed head's accuracy climbing" loading="lazy" width="666" height="640" />
  <figcaption>What training the backbone does, read by a head that is never trained. Each dot is a test image, laid out in 2D by the backbone's features at that epoch. The untrained backbone is a tangle the built head reads at 73%; as the backbone trains the classes pull apart and the same built head climbs to 86%, with no head training at any point. Rendered by <code>scripts/render_train_features_untangle_gif.py</code>.</figcaption>
</figure>
