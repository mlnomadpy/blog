...
The readout edit's invariance was a proof. The dynamics edit's cannot be, the map moved, so the only honest thing left is a measurement, and this is where the story earns its keep: over all 520 old-class test inputs, solve every fixed point under the base operator and under the edited one, and subtract.
...
The evaporation result says an edit must exist at every depth to exist at all. The tied network gets that for free; what does it cost when the weights are not shared? The run trains the honest control: the same architecture with the tying switched off, twelve untied layers, which needs 18,981 parameters to the loop's 1,733 before anyone edits anything.
