...
iterated from $z = 0$ until it stops at $z^\* = F(z^\*;x)$, then read out linearly, $\hat y = C z^\* + c$. Ours is trained on three classes, two interleaving moons and one Gaussian blob, and lands at 97.9%, 98.1%, and 100% per-class recall from 1,733 parameters shared across all depth. At the test equilibria, the measured spectral norm $\lVert J_F\rVert_2$ has mean 0.629 and max 0.904. That is a local stability audit over the tested states. It is not the uniform state-space bound a global contraction certificate would require.
...
<EquilibriumReshape caption="Row edits, answered by the whole map. The input plane is solved to equilibrium by the real operator and colored by the current decision. Toggle teach: eight anchor rows are pasted and a fourth territory appears where the new blob lives, while the recall bars for the old classes do not move, because their scores are the same computation. Toggle forget: class 2's readout row is masked and its ground floods to the neighbours. The rows card on the right is the address book: knowledge you can point at, even inside an equation." />

## A new mass, and whose landscape it lands in

Why does the timid edit feel timid? Put it back in the [landscape of wells](/blog/edit-a-network-by-hand/), which [the last post moved into state space](/blog/your-network-is-a-fixed-point/): the operator is the flow, the contraction gives it one basin, and the answer is the rest point at the bottom.
...
To do better, the mass has to go *into* the landscape. Drop the eight anchors into the shared bank, $W \leftarrow [W; c_k]$, with matching mixing columns $\gamma\,\hat c_k$, and the operator itself now has wells at the new class: the flow bends toward them, at every depth, because every depth is the same operator. That is the strong door. One paste, present everywhere. And it is the one door with no proof behind it.
...
And the natural paste breaks that local stability margin. Choosing $\gamma$ so each anchor gets a healthy pre-activation bump sends the measured slope to $\lVert J_F\rVert_2$ max **1.438** over the test probes. The measurement no longer supports locally contractive behavior around every probed equilibrium, so the raw edit is too aggressive for this solver.

The repair is one number. The edit has a single gain knob, so bisect it: find the largest fraction of the raw $\gamma$ whose measured slope stays under a ceiling of 0.98 on the test probes. The run lands on $0.462\,\gamma$; the local audit reads mean 0.646, max **0.980**. The tempered paste still does its job: new-class recall goes from 85.8% to **100%**, the margin grows from 0.55 to **3.83**, and the settle distance to the nearest anchor falls from 0.159 to 0.097. The flow now carries the new class's inputs home instead of merely labeling where they stall.

<CertificateGauge caption="Local stability as a dial. Every dot is one probe input's measured slope of F at its equilibrium, from power iteration with exact Jacobian products. Drag the gain up and the slopes near the taught class slide right. The marked gain is the largest value the run's bisection kept below 0.98 on these probes; it is an empirical operating margin, not a global contraction certificate." />
...
<OneEditEveryDepth caption="The price of 'everywhere' without weight sharing, from the real run. The loop pastes 512 numbers once and the edit exists at every turn. The untied 12-layer stack repeats the paste per layer: the numbers written grow to 6,144 while old inputs' final states shift up to 3.86. The dashed lines are the tied network's one-paste reference." />
