...
<figure class="jax-fig jax-fig-wide">
  <img src="/blog/handbuilt-detectors.gif" alt="Fashion-MNIST garments cycling, each shown beside its seven hand-built detector maps and the Yat-kernel vote" loading="lazy" width="970" height="320" />
  <figcaption>The hand-built network at work, garment by garment. Each picture is run through the seven detectors (six oriented edges plus a corner, coloured here), and the Yat kernel votes by the nearest prototype. Nothing in this path was trained; the maps and the vote are exactly the code above, run on the test set. The occasional miss (a T-shirt read as a shirt) is the honest 83.3%, not a perfect classifier. Rendered by <code>scripts/render_handbuilt_features_gif.py</code>.</figcaption>
</figure>
