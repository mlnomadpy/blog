...
<figure class="jax-fig">
  <img src="/blog/handbuilt-yat-well.gif" alt="A potential landscape with ten basins, one per class, where a garment's feature-particle rolls into the deepest well to be classified" loading="lazy" width="740" height="560" />
  <figcaption>The forward pass of the head, as a fall. Each class digs a basin whose depth is the real Yat score it gives this garment, and the feature-particle rolls into the deepest one: that is the <code>scores.argmax</code> in the module above, made physical. Most garments fall into the right well; the occasional miss (a coat read as a pullover) is the honest 83.3%, not a perfect classifier. Rendered by <code>scripts/render_handbuilt_gifs.py</code>.</figcaption>
</figure>
