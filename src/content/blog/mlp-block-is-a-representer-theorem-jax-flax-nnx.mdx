...
<figure class="jax-fig jax-fig-wide">
  <img src="/blog/yat-ffn-attribution.png" alt="Horizontal bars, each one memory slot's exact push toward the next character e, labeled with the slot's kernel weight and the tokens it writes; the bars sum to the full FFN push of 3.29 and the remaining gap reads 0.00, exact" loading="lazy" width="880" height="560" />
  <figcaption>The decomposition. For the fixed context <code>"ROMEO:\nWhat"</code>, each bar is one slot's exact share of the FFN's push toward <code>'e'</code>, k(Wᵤ, x)·(vᵤ·U[:,'e']), labeled with the tokens that slot writes; the last bar is the other 247 slots summed. The bars sum to the block's full push and the remaining gap is 0.00, exactly, because the sum is the block: no gradients, no approximation. Rendered by <code>scripts/render_yat_ffn_gifs.py</code>.</figcaption>
</figure>
