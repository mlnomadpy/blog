...
The contributions sum back to the log-risk to within $2\times10^{-6}$ on the held-out set, so a patient's score is genuinely the sum of named prototype terms.

<figure class="jax-fig jax-fig-wide">
  <img src="/blog/deepsurv-attribution.png" alt="Signed contribution bars for a patient's top prototypes, orange raising risk and blue lowering it, with the terms summing to the patient's true log-risk" loading="lazy" width="600" height="262" />
  <figcaption>A patient's log-risk decomposed into prototype contributions. Each bar is a real signed term a_u φ_u(x); orange raises risk, blue lowers it. The terms sum to the model's true h(x) = 1.47. From <code>scripts/yat_deepsurv.py</code>.</figcaption>
</figure>
...
<figure class="jax-fig jax-fig-wide">
  <img src="/blog/deepsurv-reliability.png" alt="Predicted versus observed survival for three risk tertiles at a set of follow-up horizons, for the Yat DeepSurv and the standard DeepSurv side by side" loading="lazy" width="600" height="262" />
  <figcaption>Predicted vs observed survival. Each point is a risk tertile at a follow-up horizon; the diagonal is a perfectly calibrated model. The Yat DeepSurv (left) hugs the diagonal across all three tertiles; the standard DeepSurv (right) drifts off it. Real predicted-vs-Kaplan-Meier points from <code>scripts/yat_deepsurv.py</code>.</figcaption>
</figure>
