...
*Everything here turns on one docstring comment: whatever bandwidth the solve selects is handed to the descending net unchanged. Without that line this would be a contest between two kernels rather than between two ways of fitting one, and the [explainer](/blog/you-dont-have-to-solve-a-kernel-machine/)'s finding, that the two machines agree at r = 0.95 before the descended one walks through the walls the solved one dies at, would say nothing at all. Everything else is bookkeeping around that constraint: the kernel and its Gram matrix, the Cholesky solve, the same kernel as a [Flax NNX](https://flax.readthedocs.io/en/latest/api_reference/flax.nnx/module.html) module under [Optax](https://optax.readthedocs.io/) AdamW, the timing loop that measured the wall, the minibatch run through 511,012 rows, and the conv trunk the solve can never train. The numbers come from the Kaggle runs.*
...
Training is AdamW on minibatches of 256, with two moving parts a factorization never needs. Gradient descent does not hand you a finished function; it hands you a trajectory, and you have to decide which point on it to keep. The loop scores validation every epoch and snapshots whenever it improves:
...
The snapshot pair is two lines, and the whole run depends on the `.copy()` in the first:
...
<figure class="jax-fig jax-fig-wide">
  ...
  <figcaption>The measured wall, straight off the timing loop above: exact solve on CPU float64 and GPU float32 against one SGD epoch, log-log. Gram footprints are annotated per point; at n = 64,000 the Gram alone would be 33 GB, so past 16,000 the solve has nothing left to report on this box.</figcaption>
</figure>
...
<figure class="jax-fig jax-fig-wide">
  ...
  <figcaption>The Fashion-MNIST scoreboard: the two solves (single deterministic fits) and the end-to-end model at matched and full data (mean over three seeds, individual seeds as dots). The frozen random trunk is the middle bar, below the raw pixels it was supposed to improve.</figcaption>
</figure>
