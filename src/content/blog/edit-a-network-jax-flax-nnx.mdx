...
*Teaching this network a class it has never seen is `jnp.concatenate`. Making it forget one is a boolean mask. Neither takes a gradient step, and both come with an assertion you can run. The [explainer](/blog/edit-a-network-by-hand/) makes the case that a Yat-kernel network is a list of prototype pictures; below is the list, in [Flax NNX](https://flax.readthedocs.io/en/latest/api_reference/flax.nnx/module.html), with the two edits and the Fashion-MNIST numbers they produce.*
...
If teaching and forgetting are going to be array edits, the array has to be shaped for it: one prototype to a row, and nothing about a class smeared anywhere else. A Yat unit scores an input against a prototype $W_u$ with the kernel $(W_u^\top x + b)^2 / (\lVert x - W_u\rVert^2 + \varepsilon)$, a layer is a bank of them, and the readout rows record which class each prototype speaks for. The prototypes are the rows of `W`, each row's class a row of `A`, and editing the network will turn out to be nothing more than editing those rows.
...
There is no `nnx.Linear`, no nonlinearity, and crucially no entanglement: prototype $u$ lives entirely in row $u$ of `W` and row $u$ of `A`. The decision rule is nearest-prototype within each class: class $c$ is scored by its best-matching row, $s_c(x) = \max_{u\,:\,\text{class}(u)=c} \phi_u(x)$, the `mode='max'` readout and the rule behind every number in this post. The [explainer](/blog/edit-a-network-by-hand/) writes the same structure as a pure linear readout, $s(x) = A^\top \phi(x)$, a sum over each class's rows instead of a max; that view is worth keeping (it is what a linear layer can express, and it is where the field-superposition picture comes from), but it scores a few points lower, 68% against 79% at this scale. What every edit below actually rides on is shared by both readouts: class $c$'s score is computed from class $c$'s rows and no others, so appending rows extends one class and masking rows removes one, with zero cross-talk.
...
<figure class="jax-fig jax-fig-wide">
  ...
  <figcaption>Teaching as basins forming, on real Fashion-MNIST laid out by UMAP, with zero gradient steps. Each class you teach fades in its prototype masses and a new basin grows into the landscape, claiming its territory; the accuracy in the title is the real 784-dimensional number. Because the fields superpose, the basins already present do not shift when a new one appears.</figcaption>
</figure>

## Forgetting a class is a mask

Unlearning is the same move in reverse. Find the rows assigned to the class, drop them. Because class $c$ lives only in the rows whose readout is one-hot at $c$, the deletion is exact, and this time we can hold the explainer to its promise: it said you could *diff the weights afterward to prove nothing else moved*, so snapshot them before the edit and check.
...
<figure class="jax-fig jax-fig-wide">
  ...
  <figcaption>Forgetting as a basin collapsing, by fading the Sandal prototype masses to zero. Its well vanishes and the neighbouring basins flood in to reclaim the territory it held, while the rest of the landscape does not move, the other nine classes' recall shifting by tenths of a point. Exact unlearning, as a boolean mask in the code and a removed mass in the field.</figcaption>
</figure>
