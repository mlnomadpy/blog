...
*The whole memory claim rests on one tuple. The forward rule below hands back the parameters and the final state as its residuals, and nothing indexed by depth; at 512 layers that is 3.2 MB of scratch where standard backprop reserves 674. The [explainer](/blog/backprop-without-the-memory/) argues the case; here are the block's exact inverse, the `jax.custom_vjp` that walks the trajectory backward rather than storing it, XLA's own reading of the compiled step, and the float32 cliff reproduced in twenty lines of numpy. Code blocks are quoted from the Kaggle run (bundle `kgl_blog-revmem-v1`), not paraphrased.*
...
Peak memory is easy to measure wrong, and this run takes two precautions. Each (depth, mode) point runs in a *fresh process*, because the allocator's `peak_bytes_in_use` is a process-lifetime high-water mark and later measurements inherit earlier peaks. And alongside the runtime allocator, the run reads XLA's static analysis of the compiled step, which reports how much scratch the executable reserves before it runs at all:
...
<figure class="jax-fig jax-fig-wide">
  ...
  <figcaption>Reconstruction error at every backward step (solid) against the noise budget (dashed), one color per friction. Each curve rides the slope its budget predicted, which is more than a bound would have given: the budget is describing the mechanism, not fencing it.</figcaption>
</figure>
...
*Every number above is from one run on Kaggle (bundle `kgl_blog-revmem-v1`); the float32 demonstrations are rendered from the same arithmetic the panels run live.*
