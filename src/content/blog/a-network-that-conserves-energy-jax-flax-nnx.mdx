...
*Fit two models to the same pendulum arrows and nothing separates them: 3.4e-04 for the plain field, 1.1e-04 for the one whose motion comes from a learned scalar. The difference does not exist until you integrate, and then it is the only thing there is. That gap is what the [explainer](/blog/a-network-that-conserves-energy/) is about. Below are both field models and the leapfrog classifier as [Flax NNX](https://flax.readthedocs.io/en/latest/api_reference/flax.nnx/module.html) modules, depth as a `lax.scan`, and the structural claims re-derived from scratch against the Kaggle run.*
...
<figure class="jax-fig jax-fig-wide">
  ...
  <figcaption>The trained HNN's scalar, contoured, with the rollout riding it. The trajectory stays on one stripe because the field is the gradient of this surface rotated a quarter turn, and a gradient has nothing to say along its own level set.</figcaption>
</figure>
...
<CiteAs slug="a-network-that-conserves-energy-jax-flax-nnx" title="Building the Hamiltonian-Step Net in JAX/Flax NNX" pubDate="2026-07-16" />
