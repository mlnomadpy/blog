...
This is the same physical world this series has used since [the attractor fields of Arc C](/blog/edit-a-network-by-hand), landscapes, wells, things that roll, and this time the landscape is not a reading of the model, it *is* the model: training a classifier now means sculpting a terrain such that marbles thrown from different classes swing to separable places. And unlike the pendulum's phase plane, this state space is eight-dimensional, so when the panels below show you a plane of it, that plane is a window, not the room.
...
But the scoreboard is a trap this arc has sprung before. [D1's exactness ceiling](/blog/skip-connections-are-half-of-newton) did not show up in accuracy either, and [D2's ledger](/blog/transformers-with-a-velocity-ledger) tied on quality while rebuilding the road. The question a conservation law answers is not "how often is the argmax right?" It is: what is true of the computation *between* the encoder and the head? So look between.
