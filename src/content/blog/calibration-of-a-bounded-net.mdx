...
For the network this series has been building, I expected the answer to be yes, and I would have bet on it. The Yat-kernel MLP exposes [input-space prototypes and exact layer-local terms](/blog/what-a-finite-kernel-buys-an-mlp/), and off its training distribution the kernel field can go quiet instead of extrapolating a large raw response. A network built from bounded, local similarity scores might also produce probabilities you can trust. This experiment tests that inference.

So I measured it. Same twins as before, trained the same way on Fashion-MNIST, with the same softmax head on both so that confidence means exactly the same thing for each; three seeds, and every number below is printed by that run.
...
And before the diagram loads, let the series make its prediction out loud. Bounded wells, local similarity scores, a field that hushes on anything foreign: every property this network is known for says its staircase should hug the diagonal while the black box sags below it. Both networks are binned from their actual test logits. Hover any bin: it tells you how many predictions it holds, what they claimed, and what they delivered.

<ReliabilityStaircase caption="Reliability diagrams for the Yat MLP (left) and the matched ReLU MLP (right), from 4000 exported test logits of the seed-0 run: softmax, take the max, drop into 15 equal-width bins. The bar is delivered accuracy, the tick is claimed confidence, and the shaded strip between them is the overconfidence gap. The dashed diagonal is a perfectly honest forecaster." />
...
Verdict, in-distribution: **the bounded kernel does not buy calibrated probabilities.** Calibration lives in the probability head, and both networks have the same head; the Yat layer just feeds it wilder numbers.

## Off the diet

But the "knows when it doesn't know" case was made off-distribution, so before conceding the story, feed both networks something neither has ever seen: MNIST digits, handwriting shown to a garment classifier. Every answer is now meaningless, so every confident answer is a lie. Who lies?

<ConfidenceLedger caption="Max-confidence histograms from the exported logits: each net's claims on 4000 Fashion-MNIST test images (filled) against 2000 MNIST digits it has never seen (outlined step). The dashed line marks 90% confidence. In distribution both nets pile against the right wall; off distribution the question is how much of the pile refuses to move." />

Here the Yat net partially supports the [raw-response hypothesis](/blog/what-a-finite-kernel-buys-an-mlp/): in the histograms above its median claim on digits drops to **0.45** (the ReLU net's: 0.65), and averaged over seeds it calls a digit with more than 90% confidence **12.5%** of the time against the ReLU net's **21.2%**. It is quieter on average, not silent. On seed 2 the Yat net's above-90% rate on digits is 22.6%, as high as a typical ReLU run.

That looks like a retreat from that post's near-clean separation, until you notice what it actually measured. It never asked the softmax anything. It read the **kernel scores themselves** and thresholded their maximum. Different number. Very different number.
