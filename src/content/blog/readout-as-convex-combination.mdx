...
Each $r_u$ is a one-neuron message to the residual stream: the direction, token-logit effect, downstream feature, or circuit ingredient that unit $u$ contributes when it turns on. The readout is the superposition of those messages.
...
The geometry depends on the coefficients, and on two yes/no questions about them: are they **nonnegative**, and do they **sum to one**? The two constraints are independent, so together they carve the readout into four regimes, each with its own reachable set.
...
Nonnegativity buys you the hull; summing to one pins you to the affine flat; both together give the simplex.
...
<NonIdentifiability caption="Non-identifiability of the convex decomposition, made literal. The black point y is held fixed while the coefficients morph through a whole family of convex decompositions of it, each a valid set of nonnegative shares summing to one. The cycle passes through sparse decompositions that use only three prototypes (Carathéodory's bound in the plane) and dense ones that spread mass across more. Because the set of valid decompositions is convex, interpolating between any two stays valid, so y is provably constant, and the readout under the point confirms it frame by frame. 'How much did prototype u contribute?' has many equally-correct answers." />
...
Write that block down and you have rediscovered two known architectures. A softmax over stored prototypes followed by a weighted readout is one retrieval step of a **modern Hopfield network** (Ramsauer et al., 2020), whose update rule is the attention mechanism; its three regimes, converging to a single prototype, to a metastable average over a few, or to the global mean, are the temperature regimes of the softmax. And if a *sparse* convex code is wanted instead of a dense one, **sparsemax** and **entmax** (Martins & Astudillo, 2016; Peters et al., 2019) replace the softmax with a projection onto the simplex that sets most coefficients to exactly zero, a readout that is convex *and* sparse, so only a handful of prototypes are ever mixed.
...
The output projection in an MLP is a dictionary of messages the hidden layer can write to the residual stream. That one shift is what the taxonomy above hangs on, and it is worth having in one sentence:

> An MLP block is a detector bank followed by a prototype readout. Attention mixes token-indexed prototypes. The MLP mixes neuron-indexed prototypes. Both have a readout; what separates them is whether the coefficients form a clean probability distribution.

Once you see $W_{\text{out}}$ this way, the natural unit of analysis is a pair rather than a neuron activation on its own: the detector that made the coefficient, and the output prototype that coefficient writes.
...
<KernelQuadrant caption="Positive-definite and pointwise-nonnegative are independent axes, and only the second is what a convex readout needs. For each kernel the figure computes two things: the profile κ(x₀, ·), where a dip below zero means it is not nonnegative, and the smallest eigenvalue of a Gram matrix, where λ_min < 0 means it is not positive definite. The linear kernel is PSD but signed; the boxcar is nonnegative but not PSD; the Gaussian and Yat kernels occupy the useful top-right corner, nonnegative and Mercer at once." />

With such a kernel, the readout is exactly a convex combination of the output prototypes $r_u$, a Nadaraya–Watson estimator over a learned prototype set. The detector bank stops being an arbitrary feature extractor and becomes a similarity to named input prototypes, and the convex reading is then the mechanism itself rather than an interpretation laid over one. A later post carries these kernel coefficients into a working transformer's feed-forward block and reads the result as a representer theorem: [The MLP Block Is a Representer Theorem](/blog/mlp-block-is-a-representer-theorem/). It lands in a different cell of the 2x2 than you might guess. It keeps the kernel scores and drops the normalization, so its coefficients are signed, and the attribution it gives you is exact but linear rather than convex. You get an edit that lands where the algebra says it will, and you give up contribution mass, which is the trade this taxonomy exists to make visible.

Normalization has a boundary: when every $\kappa(x,p_v)$ is tiny, the denominator approaches zero and the normalized weights become ill-conditioned. Fast-decaying Gaussian kernels reach that regime sooner than heavier-tailed inverse-multiquadric-style gates. Convexity holds wherever the denominator is positive; numerical stability off the prototype set depends on the kernel tail and the chosen floor.

This is the whole construction in one picture. Every hidden unit pairs an input-side prototype $p_u$ with an output-side prototype $r_u$. The query $x$ lives in the input space on the left; its kernel similarities to the $p_u$ become the coefficients; those same coefficients mix the $r_u$ in the residual stream on the right. Drag $x$ anywhere and the readout $y$ can never escape the output hull, because the way the coefficients are formed leaves them no other option.
...
Attention gets its convexity from the softmax. The MLP could get it from a kernel over prototypes, in place of a linear map into an activation, and then the contribution question this post opened with would have one answer instead of many.
