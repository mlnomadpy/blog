...
Write the hidden activation vector of a transformer MLP as $a(x) \in \mathbb{R}^{m}$ and write the columns of $W_{\text{out}}$ as output vectors $r_1,\ldots,r_m$ in the residual stream. Then
...
The first linear map plus activation decides **which hidden features are active**. The second linear map decides **what each active feature writes**. Its $u$-th column is the vector written when hidden unit $u$ fires alone:
...
The direction is still a convex mixture; the norm is an intensity. The reachable set is the **conical hull** — the cone the prototypes generate from the origin. This is the natural reading for a ReLU-style MLP: pick a point in the hull, then scale how loudly you write it into the residual stream.
...
And even in the convex regime there is a subtlety worth stating plainly. Because a transformer has far more hidden units than residual dimensions ($m \gg d$), the prototypes are an **overcomplete** dictionary — so by Carathéodory's theorem the same output $y$ can be written as a convex combination of them in infinitely many ways, in fact using as few as $d+1$ at a time. The coefficient vector $a(x)$ the network produces is therefore *a* convex decomposition, not *the* one. "Unit $u$ contributed 30%" is a fact about the mechanism that generated $a(x)$, not a fact recoverable from $y$ alone. Contribution mass is a property of the readout map, not of its output.
...
<NonIdentifiability caption="Carathéodory non-identifiability, made literal. The black point y is held fixed while the coefficients morph through a whole family of convex decompositions of it — each a valid set of nonnegative shares summing to one. The cycle passes through sparse decompositions that use only three prototypes (Carathéodory's bound in the plane) and dense ones that spread mass across more. Because the set of valid decompositions is convex, interpolating between any two stays valid, so y is provably constant — recomputed live in jax-js each frame and shown unchanged in the readout. 'How much did prototype u contribute?' has many equally-correct answers." />
...
The gate also tells you what convexity costs. A pure convex readout can only reach the convex hull of $\{r_u\}$; adding the nonnegative gate $g(x)$ extends the reach to the conical hull, the cone the prototypes generate. Neither covers all of $\mathbb{R}^d$. To write in *every* direction with nonnegative coefficients, the prototypes must form a **positive spanning set** — and a positive spanning set of $\mathbb{R}^d$ needs at least $d+1$ vectors in general position. So the interpretability of nonnegative coefficients is not free: it either restricts the block's output cone or forces redundancy in the prototype dictionary. Signed coefficients buy the full span back, at the price of the clean mass interpretation. That is the whole trade in one sentence.

The design question is whether that constraint is worth the interpretability. Attention already made that bargain. Its output is a convex mix before the output projection. The MLP historically did not; it bought flexibility by allowing arbitrary signed linear readout. The cost is that its contribution semantics are weaker.
...
The kernels worth reaching for satisfy **both**. The exponential kernel behind softmax attention, the Gaussian, and the Yat kernel $\kappa(x,y)=(x^\top y)^2/(\lVert x-y\rVert^2+\varepsilon)$ are each nonnegative *and* positive definite — Yat by a Schur product of a squared-linear numerator (PSD, and manifestly $\ge 0$) with an inverse-multiquadric factor (PSD). That intersection is the corner you want: a similarity that is simultaneously a legitimate Mercer kernel and a source of convex weights.

<KernelQuadrant caption="Positive-definite and pointwise-nonnegative are independent axes, and only the second is what a convex readout needs. For each kernel the figure computes, live, the profile κ(x₀, ·) — a dip below zero means it is not nonnegative — and the smallest eigenvalue of a Gram matrix (via the engine's symmetric eigendecomposition) — λ_min < 0 means it is not positive definite. The linear kernel is PSD but signed; the boxcar is nonnegative but not PSD; the Gaussian and Yat kernels occupy the useful top-right corner, nonnegative and Mercer at once." />
