...
print("zero-training accuracy:", round(float((votes.argmax(-1) == yte).mean()) * 100, 1))   # 67.5
```

Two thirds of the test set labelled correctly, no optimizer, no backprop. (The linear readout can only *sum* the votes, which caps this build at 67.5%; replacing the sum with a nearest-prototype `max` reaches 78.2%, but a linear layer cannot express a max. Built from the full training set instead of this 20k subset, the same recipe lands at 68.5% and 79.4%, the pair the explainer's panel shows; `scripts/yat_fmnist_export.py` prints all four.) The point stands: classification is not something training installs in a black box. It is what the architecture is, and a good engineered selection of prototypes makes it work on its own.
