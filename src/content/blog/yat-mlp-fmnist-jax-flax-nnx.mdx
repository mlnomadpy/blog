...
A Yat unit measures the input against a learned prototype $W_u$ with the kernel $(W_u^\top x + b)^2 / (\lVert x - W_u\rVert^2 + \varepsilon)$. A layer is a bank of them, and there is no activation function. The whole model is that layer plus a linear readout.
