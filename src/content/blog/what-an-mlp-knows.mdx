...
<YatKernel caption="Where the previous contrast settled that the unit has a peak at all, this panel is about the peak's shape. The Yat kernel section K(·, W) over a 2D input space falls off as 1/‖x − W‖² (regularised by ε) outside the peak. Click anywhere to move W; the ε slider is the selectivity dial, smaller ε means a sharper, more localised unit. This is the RKHS section attached to one prototype; the full layer is a weighted sum of m such sections, one per unit." />
