...
description: 'The hidden codebook inside representation learning: why collapse happens, why opposition is a trap, why class means form a simplex, and why the Welch bound sets the best geometry when too many concepts share too few dimensions.'
...
That repulsion prevents collapse is the boring half. The interesting half is the shape it chooses. Why do the centroids settle at those angles? Why not push every class to the opposite side of every other class? Why does the good solution look so rigid?
...
Cosine $-1$ is minimum signed similarity, while an antiparallel pair still spans one line. Cosine $0$ instead marks zero projection. [Untangling the Moons](/blog/untangling-the-moons/) separates those questions; here the issue belongs to the whole codebook rather than one pair.
...
This is the first payoff. Go back to the opening animation and the centroids were not just spreading; they were converging on the only centered arrangement in which every class gets the same deal, and there is exactly one such arrangement for each $C$.
...
The bound is universal, but equality is special. When an **equiangular tight frame** exists for that $(C, d)$, it attains the floor: equiangular means the crosstalk is shared evenly instead of dumped onto a few unlucky pairs, and tight means the vectors use the whole space evenly, which is the compromise the three constraints demanded. Such frames do *not* exist for every $(C, d)$; when none does, the best packing can only approach the same floor rather than sit on it.
...
What happens to a classifier trained long past zero training error? The natural guess is drift into ever more idiosyncratic structure. When [Papyan, Han, and Donoho](https://arxiv.org/abs/2008.08186) looked in 2020, they found the opposite. The late phase simplified:
...
A network with a hundred million parameters, given a hundred epochs past the point of caring, spends them arriving at a shape a radio engineer could have drawn on a napkin in 1974.

## Contrastive learning is codebook design with two forces

And what about training without labels? Contrastive learning has no class means to collapse, yet it faces the same crowded channel. [Wang and Isola](https://proceedings.mlr.press/v119/wang20k.html) gave it a clean two-force description:
...
"Well separated" was never enough to specify a good latent space. It is a codebook, and it has to satisfy three constraints at once:
