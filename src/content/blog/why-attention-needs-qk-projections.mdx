...
There is a clean way to see exactly what separate $Q$ and $K$ add over a shared projection. Any matrix splits into a symmetric and an antisymmetric piece,
...
**It is role-aware.** The two arguments are not interchangeable unless $B$ is symmetric. Attention can learn "A asks for B" without also learning "B asks for A."

**It is compositional.** If a token contains several features, the score is the sum of pairwise feature interactions selected by $B$. This lets one head score a particular cross-feature relation instead of a whole-token similarity.

**It is low-rank by construction.** Since $B=W_QW_K^\top$, its rank is at most the head dimension $d_k$. The head does not learn every possible interaction in the residual stream. It learns a compressed relation: a small set of query features matched against a small set of key features.
